...
sections <- unique(stu_sections$section)

sort.list(sections)

i <- 1
stus_cur_section <- stu_sections[stu_sections$section == sections[i], ]

sum(stus_cur_section$training_set)

##########################################


df <- data_raw100_assessment_all


# assign profile names (P1 as most populous)
 # profile_names <- tibble(long_name = unique(all_assignments$profile),
 #                         count = NA,
 #                         short_name = NA)
 # 
 # for (i in 1:nrow(profile_names)) {
 #  profile_names$count[i] <- sum(all_assignments$profile == profile_names$long_name[i])
 # }                              
 # 
 # sort(x = profile_names$count, decreasing = T)
